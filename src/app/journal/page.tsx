...
import { Plus, BookOpen, Clock, Calendar, BarChart3, AlertCircle, Trash2 } from 'lucide-react';
...
export default function JournalPage() {
  ...
  return (
    <div className="flex-grow bg-transparent px-4 py-8 max-w-6xl mx-auto w-full flex flex-col gap-6 relative z-10 text-black">
      ...
      {showLogForm && (
        <div className="bg-white border-[3px] border-black rounded-2xl p-6 glass-panel shadow-[5px_5px_0px_#000000] text-black">
          ...
          <form onSubmit={handleLogSession} className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">Topic Studied</label>
                <input
                  type="text"
                  required
                  value={topic}
              ...
              {/* LeetCode list */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">
                ...
                <textarea
                  ...
                  rows={2}
                  className="bg-white border-2 border-black rounded-lg px-3 py-2 text-sm text-black focus:outline-none focus:bg-neo-yellow shadow-[1px_1px_0px_#000000] resize-none"
                />
              </div>

              {/* Notes */}
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">Study Notes / Insights</label>
                <textarea
                  value={notes}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">Questions Difficulty Split</label>
                <div className="grid grid-cols-3 gap-2">
                  {/* Easy */}
                  <div className="flex flex-col gap-1 bg-neo-gray border-2 border-black p-2 rounded-xl items-center shadow-[1px_1px_0px_#000000]">
                    <span className="text-[9px] font-black text-black">EASY</span>
                    <input
                      type="number"
                      min={0}
                      value={easyCount}
                      onChange={(e) => setEasyCount(Math.max(0, parseInt(e.target.value) || 0))}
                      className="w-12 text-center bg-white border-2 border-black rounded px-1.5 py-1 text-sm font-mono text-black font-extrabold focus:outline-none focus:bg-neo-yellow"
                    />
                  </div>

                  {/* Medium */}
                  <div className="flex flex-col gap-1 bg-neo-gray border-2 border-black p-2 rounded-xl items-center shadow-[1px_1px_0px_#000000]">
                    <span className="text-[9px] font-black text-black">MEDIUM</span>
                    <input
                      type="number"
                      min={0}
                      value={mediumCount}
                  ...
                  {/* Hard */}
                  <div className="flex flex-col gap-1 bg-neo-gray border-2 border-black p-2 rounded-xl items-center shadow-[1px_1px_0px_#000000]">
                    <span className="text-[9px] font-black text-black">HARD</span>
                    <input
                      type="number"
                      min={0}
                      value={hardCount}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">Time Spent (Minutes)</label>
                  <input
                    type="number"
                    min={10}
                    required
                    ...
                    className="bg-white border-2 border-black rounded-lg px-3 py-2 text-sm text-black focus:outline-none focus:bg-neo-yellow shadow-[1px_1px_0px_#000000] font-mono"
                  />
                </div>
                
                <div className="flex flex-col gap-1.5">
                  <label className="text-[10px] font-bold text-black/60 uppercase tracking-wider">Grind Date</label>
                  <input
                    type="date"
                    required
                    value={date}
              ...
              <button
                type="submit"
                className="mt-auto w-full py-2.5 rounded-xl border-2 border-black bg-neo-green text-black font-bold font-orbitron text-xs uppercase tracking-widest hover:bg-neo-green/80 shadow-[3px_3px_0px_#000000] active:translate-x-[3px] active:translate-y-[3px] active:shadow-none transition-all cursor-pointer"
              >
      ...
      <section className="flex flex-col gap-4">
        ...
        {loading ? (
          <div className="text-center py-8 text-xs text-black/55 font-bold font-mono">LOADING JOURNAL PROTOCOL...</div>
        ) : sessions.length === 0 ? (
          <div className="bg-white border-[3px] border-black rounded-2xl py-16 text-center text-xs text-black/55 font-bold tracking-wider flex flex-col items-center gap-2 shadow-[4px_4px_0px_#000000]">
            <AlertCircle size={20} />
            <span>NO DSA JOURNAL SESSIONS REGISTERED FOR THIS USER YET.</span>
          </div>
        ) : (
          <div className="flex flex-col gap-4 max-h-[500px] overflow-y-auto pr-1.5 scrollbar-hide">
            {sessions.map((session) => (
              <div 
                key={session.id}
                className="bg-white border-[3px] border-black rounded-xl p-5 hover:translate-y-[-2px] transition-all flex flex-col gap-3 relative overflow-hidden group shadow-[3px_3px_0px_#000000] text-black"
              >
                {/* Visual Accent Indicator */}
                <div className="absolute top-0 left-0 bottom-0 w-2 bg-neo-blue border-r-2 border-black" />
                
                {/* Header row */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 pl-2">
                  <div className="flex items-center gap-3">
                    <span className="font-orbitron font-extrabold text-sm tracking-wide text-black">
                      {session.topic}
                    </span>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-black/60 font-mono font-bold">
                    ...
                    {activeUser && activeUser.id === session.user_id && (
                      <button
                        ...
                        className="p-1 rounded-md border-2 border-black bg-white text-black hover:bg-rose-200 shadow-[1px_1px_0px_#000000] active:translate-y-[1px] active:shadow-none transition-all cursor-pointer"
                ...
                {session.leetcode_questions.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 items-center pl-2">
                    <span className="text-[9px] font-black uppercase tracking-wider text-black/50 mr-1">Questions:</span>
                    {session.leetcode_questions.map((q, idx) => (
                      <span 
                        key={idx} 
                        className="text-[10px] font-semibold text-black px-2 py-0.5 rounded border-2 border-black bg-neo-purple shadow-[1px_1px_0px_#000000]"
                      >
                        {q}
                      </span>
                    ))}
                ...
                <div className="flex gap-4 text-[9px] font-bold font-mono pl-2">
                  {session.difficulty_easy > 0 && <span className="text-black bg-neo-green/10 border border-black/25 px-1.5 py-0.5 rounded-sm">EASY: {session.difficulty_easy}</span>}
                  {session.difficulty_medium > 0 && <span className="text-black bg-neo-yellow/15 border border-black/25 px-1.5 py-0.5 rounded-sm">MEDIUM: {session.difficulty_medium}</span>}
                  {session.difficulty_hard > 0 && <span className="text-black bg-neo-peach/20 border border-black/25 px-1.5 py-0.5 rounded-sm font-black">HARD: {session.difficulty_hard}</span>}
                </div>

                {/* Study Notes */}
                {session.notes && (
                  <p className="text-xs text-black/75 leading-relaxed border-t-2 border-black/10 pt-2.5 mt-0.5 italic pl-2">
                    &quot;{session.notes}&quot;
                  </p>
                )}
              </div>
